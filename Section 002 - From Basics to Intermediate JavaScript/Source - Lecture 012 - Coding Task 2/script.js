...
var passing = [91, 81, 71, 61, 51];

function calcTotalScore(s1, s2) {
	var total = s1 + s2;
...
function calcFinal() {
	for (var i = 0; i < students.length; i++) {
		students[i].total = calcTotalScore(students[i].score1, students[i].score2);

		if (students[i].total >= 51) {
			console.log(students[i].name + ' passed an exam');

			for (var x = 0; x < passing.length; x++) {
				if (students[i].total >= passing[x]) {
					console.log('Student scored ' + students[i].total + ' and got ' + degrees[x] + ' degree.');
			...
			console.log(students[i].name + ' scored ' + students[i].total + ' and failed an exam.');
			console.log('')
		}
